...
def _make_private_directory(path: Path) -> None:
    ...
    if not account:
        raise unittest.SkipTest("current Windows account is unavailable")
    ...
    if result.returncode != 0:
        raise unittest.SkipTest("unable to create generated private DACL")
...
def _make_private_file(path: Path) -> None:
    ...
    if result.returncode != 0:
        raise unittest.SkipTest("unable to protect generated private file")
